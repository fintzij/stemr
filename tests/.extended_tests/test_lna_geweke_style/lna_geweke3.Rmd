...
for(k in 2:iterations) {
        ...
        if(k%%100 == 0) {
                cat(paste0("Prevalence iteration ", k), sep = "\n", file = "lna_geweke_linear.txt", append = TRUE)
        }
...
I_exact <- gillespie_paths[,"I",] - matrix(path$path$drift_process[,2], nrow = nrow(I_stemr), ncol = ncol(I_stemr))
...
print(ggplot(results_comp, aes(x = time, y = mean, colour = method, linetype = method)) + geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) + geom_line()  + labs(x = "time", y = "I", title = "Pointwise distribution of lna paths from prevalence data - mean and MCSE bands"))
...
results_quantile[results_quantile$method == "stemr", 3:5] <- results_quantile[results_quantile$method == "stemr", 3:5]

print(ggplot(results_quantile, aes(x = time, y = med, fill = method, linetype = method)) + geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) + geom_line()  + labs(x = "time", y = "I", title = "Pointwise distribution of lna paths from prevalence data - median and 50% quantile bands"))
