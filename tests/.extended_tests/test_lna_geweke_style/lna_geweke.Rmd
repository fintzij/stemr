...
gillespie_paths <- exact_paths$paths

I_stemr <- stemr_paths[,"I",]
I_exact <- gillespie_paths[,"I",]

library(mcmc)
library(ggplot2)

results_comp <- data.frame(method = rep(c("stemr", "gillespie"), each = 53),
                           time = rep(0:52, 2), mean = 0, mcse = 0, lower = 0, upper = 0)
results_comp[results_comp$method == "stemr","mean"] <- rowMeans(I_stemr)
results_comp[results_comp$method == "gillespie","mean"] <- rowMeans(I_exact)
...
results_comp[results_comp$method == "stemr","mcse"] <- sqrt(stemr_vars)/sqrt(nrow(I_stemr))
results_comp[results_comp$method == "gillespie","mcse"] <- apply(I_exact, 1, sd)/sqrt(nrow(I_exact))
...
results_comp[results_comp$method == "gillespie", "lower"] <- results_comp[results_comp$method == "gillespie","mean"] - 
                                                                     1.96 * results_comp[results_comp$method == "gillespie","mcse"]
results_comp[results_comp$method == "gillespie", "upper"] <- results_comp[results_comp$method == "gillespie","mean"] + 
                                                                     1.96 * results_comp[results_comp$method == "gillespie","mcse"]
# produce the plot
pdf("lna_geweke_prevalence.pdf")
print(ggplot(results_comp, aes(x = time, y = mean, colour = method)) + geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) + geom_line()  + labs(x = "time", y = "I", title = "Pointwise prevalence from prevalence data - mean and MCSE bands"))


# quantiles plot
results_quantile <- data.frame(method = rep(c("stemr", "gillespie"), each = 53),
                           time = rep(0:52, 2), 
                           med = c(apply(I_stemr, 1, quantile, 0.5), apply(I_exact, 1, quantile, 0.5)),
                           lower = c(apply(I_stemr, 1, quantile, 0.025), apply(I_exact, 1, quantile, 0.025)), 
                           upper = c(apply(I_stemr, 1, quantile, 0.975), apply(I_exact, 1, quantile, 0.975)))
results_quantile[results_quantile$method == "stemr", 3:5] <- exp(results_quantile[results_quantile$method == "stemr", 3:5])

print(ggplot(results_quantile, aes(x = time, y = med, colour = method)) + geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) + geom_line()  + labs(x = "time", y = "I", title = "Pointwise prevalence from prevalence data - median and 95% quantile bands"))

# plot datasets
dat_stemr <- array(unlist(datasets), dim = c(53, 2, length(datasets)))[,2,]
dat_exact <- exact_paths$datasets[,2,]

dat_comp <- data.frame(method = rep(c("stemr", "gillespie"), each = 53),
                           time = rep(0:52, 2), mean = 0, mcse = 0, lower = 0, upper = 0)
dat_comp[dat_comp$method == "stemr","mean"] <- rowMeans(dat_stemr)
dat_comp[dat_comp$method == "gillespie","mean"] <- rowMeans(dat_exact)
...
dat_comp[dat_comp$method == "gillespie","mcse"] <- apply(dat_exact, 1, sd)/sqrt(nrow(I_exact))
...
dat_comp[dat_comp$method == "stemr", "upper"] <- dat_comp[dat_comp$method == "stemr","mean"] + 
                                                                     1.96 * dat_comp[dat_comp$method == "stemr","mcse"]
dat_comp[dat_comp$method == "gillespie", "lower"] <- dat_comp[dat_comp$method == "gillespie","mean"] - 
                                                                     1.96 * dat_comp[dat_comp$method == "gillespie","mcse"]
dat_comp[dat_comp$method == "gillespie", "upper"] <- dat_comp[dat_comp$method == "gillespie","mean"] + 
                                                                     1.96 * dat_comp[dat_comp$method == "gillespie","mcse"]
