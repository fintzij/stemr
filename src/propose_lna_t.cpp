...
//' @export
// [[Rcpp::export]]
Rcpp::List propose_lna_t(const arma::rowvec& lna_times,
        ...
        for(int j=0; j < (n_times-1); ++j) {
                ...
                try{
                        ...
                        log_lna = lna_drift + (svd_U * svd_V.t()) * (draws.col(j) / arma::sqrt(Rcpp::as<arma::vec>(Rcpp::rchisq(n_events, df))/df)); // map the LNA draws
