...
//' @export
// [[Rcpp::export]]
Rcpp::List propose_lna_semi_t(const arma::rowvec& lna_times,
                       ...
                       double step_size,
                       double df,
                       const Rcpp::LogicalVector& rate_orders,
                       SEXP lna_pointer,
        ...
        arma::vec chisq_draws(n_events, arma::fill::ones);
        ...
        for(int j=0; j < (n_times-1); ++j) {
                ...
                try{
                        ...
                        for(int s = 0; s < n_events; ++s) {
                                if(rate_orders[s]) chisq_draws[s] = Rcpp::rchisq(n_events, df)[0] / df;
                        }

                        log_lna = lna_drift + (svd_U * svd_V.t()) * (draws.col(j) / arma::sqrt(chisq_draws)); // map the LNA draws
