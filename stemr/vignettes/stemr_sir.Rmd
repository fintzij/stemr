...
mcmc_kernel <-
        kernel(
                ...
                stop_adaptation = 1e2, # number of iterations for adaptation,  1e4 in coverage simulation
                sigma = covmat,
                scale_constant = 0.25,
...
# for initializing the parameters at random functions
...
res <-
  stem_inference(stem_object = stem_object,
                 method = "lna", # or set to "lna"
                 iterations = 3.5e2, # total number of iterations, set to 3.5e4 for the paper
                 thin_params = 10,
                 thin_latent_proc = 10,
                 initialization_attempts = 500,
                 priors = priors,
                 mcmc_kernel = mcmc_kernel,
                 ess_args = # settings elliptical slice sampling for LNA
                   ess_settings(n_ess_updates = 1,
                        ess_warmup = 100,
...
```{r pomp_fit, echo = TRUE, warning=FALSE}
require(pomp)
S0 <- 1e4 - 10
I0 <- 10
R0 <- 0
...
sir.step<-paste0("
  double rate[2];
  double dN[2];
  rate[0]=exp(log(exp(log_R0)+1) + log_mu - log(",popsize,"))*I; // Infection rate
