...
state_initializer <-
  list(stem_initializer(c(S = 1e4-15, I = 10, R = 5),
                        fixed = T, 
                        prior = c(1e4-15, 10, 5)))
adjacency <- NULL
tcovar <- cbind(time = 0:30,
                beta_t = rlnorm(31, log(2.1), 0.25) / 1e4)
parameters = c(mu = 1, omega = 1/4, rho = 0.5, phi = 50)
constants <- c(t0 = 0)
t0 <- 0; tmax <- 30;
...
# simulate dataset
sim_lna <- simulate_stem(stem_object = stem_object, method = "lna")
...
constants <- c(t0 = 0)
popsize = 1e4; log_popsize = log(popsize)
t0 <- 0; tmax <- 30;

# RW in terms of log(R0), parameterized by differences
foi_rlnorm <- function(parameters, draws, log_pop = log_popsize) {
  
  log_R0_t <- log(2.1) + parameters["sigma"] * draws
...
to_estimation_scale <- function(params_nat) {
  return(c(
    -log(params_nat[1]),        # mu            -> log(mu)
    -log(params_nat[2]),        # omega         -> log(omega)
...
from_estimation_scale <- function(params_est) {
  return(c(
    exp(-params_est[1]),                   # log(mu)        -> mu
    exp(-params_est[2]),                   # log(omega)     -> omega
...
logprior = function(params_est) {
    dnorm(params_est[1], 0.05, 0.5, log = TRUE) +                    # log(1/mu) 
    dnorm(params_est[2], 1.35, 0.5, log = TRUE) +                    # log(1/omega)
    dnorm(params_est[3], mean = logit(0.5), sd = 0.5, log = TRUE) +  # logit(rho)
    dexp(exp(params_est[4]), 5, log = TRUE) + params_est[4] + 
    dexp(exp(params_est[5]), 20, log = TRUE) + params_est[5]
}
...
mcmc_kern <-
  mcmc_kernel(
    parameter_blocks = 
      list(parblock(
        pars_nat = c("mu", "omega", "rho", "phi", "sigma"),
        pars_est = c("log_mu", "log_omega", "logit_rho", "log_phi", "log_sigma"),
        priors = priors,
        # alg = "mvnss",
        alg = "mvnmh",
        sigma = diag(0.01, 5),
    ...
    lna_ess_control = lna_control(bracket_update_iter = 0,
...
max_adaptation =
  max(sapply(param_blocks, function(x) x$control$stop_adaptation))
...
while(keep_going & iter <= iterations) {
  ...
  #     if (iter == max_adaptation || iter == iterations) {
  #       
  ...
  if(abs(path$data_log_lik) > 1e4 | is.nan(abs(path$data_log_lik))) {
    keep_going = FALSE
  ...
  if(record_sample && iter %% thinning_interval == 0) {
    ...
    if(abs(mcmc_samples$data_log_lik[rec_ind]) > 1e4 | 
