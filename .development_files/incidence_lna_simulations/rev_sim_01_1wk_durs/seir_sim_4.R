...
# from an SEIR model with Ebola-like dynamics. 
#
# Settings: 
# - R0 = 2
...
# - P  = 5000
# - X(t_0) = (S_0 = 4990, E_0 = 5, I_0 = 5, R_0 = 0)
...
# create stem model
popsize = 5e3 # population size

true_pars =
      c(R0         = 2, # basic reproduction number
        latent_dur = 1, # latent period duration = 1 week
...
if(length(runs) != 0) {
  ...
  # add buffer to get a whole month
  if(max(inds) %% 4 != 0) {
    inds = seq(9, ceiling(max(inds) / 4) * 4)
  }
...
dat_m <- 
  cbind(time = seq(0, max(dat[,1]), by = 4)[-1],
        E2I = 0)

for(s in seq_along(dat_m[,1])) {
  dat_m[s, 2] <- sum(dat[seq(dat_m[s,1], length.out = 4, by=-1), 2])
}

# overwrite weekly data
dat <- dat_m
...
to_estimation_scale = function(params_nat) {
  c(log(params_nat[1] * popsize / params_nat[3]), # (beta,mu,N) -> log(R0)
    -log(params_nat[2]),                          # omega, mu -> log(omega)
...
from_estimation_scale = function(params_est) {
  c(exp(params_est[1] - params_est[3] - log(popsize)), # (log(R0), log(mu), N) -> beta = exp(log(R0) + log(mu) - log(N))
    exp(-params_est[2]),                               # log(omega) -> omega, mu
...
logprior =
  function(params_est) {
    dnorm(params_est[1], log(1.8), 0.5, log = TRUE) + 
      dnorm(params_est[2], 0, 0.82, log = TRUE) + 
      dnorm(params_est[3], 0, 0.82, log = TRUE) + 
        ...
        mcmc_kernel(
          parameter_blocks = 
              list(parblock(
                  ...
                  pars_est = c("log_R0", "log_latent_dur", "log_infecdur", "logit_rho", "log_phi"),
                  priors = priors,
                  alg = "mvnmh",
                  sigma = diag(0.01, length(parameters)),
                  initializer = par_initializer,
                  control = mvnmh_control(stop_adaptation = 2.5e4))),
...
# extract results
posts <- do.call(rbind, lapply(fits, function(x) x$results$posterior$parameter_samples_est))
res   <- apply(posts, 2, quantile, c(0.025, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.975)) 

saveRDS(res, file = paste0("seir_",obs_interval,"_",replication,".Rds"))
