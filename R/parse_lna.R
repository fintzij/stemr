...
parse_lna <- function(rates, hazard_fcns, rate_derivs, messages = TRUE) {
        ...
        jacobian_args <- hazard_args <- "double t, const Rcpp::NumericVector& state, const Rcpp::NumericVector& parameters"
        jacobian_inds <- matrix(c(rep(seq(0, n_rates - 1), each = n_compartments), rep(seq(0, n_compartments - 1), n_rates)), ncol = 2)
        hazard_fcn    <- paste0("Rcpp::NumericVector hazards(", n_rates,");")
        jacobian_fcn  <- paste0("Rcpp::NumericMatrix jacobian(", n_rates, ", ", n_compartments, ");")
        ...
        hazard_code <- paste("// [[Rcpp::depends(Rcpp)]]",
                             "#include <Rcpp.h>",
                             "using namespace Rcpp;",
                             paste0("Rcpp::NumericVector HAZARD(",hazard_args,") {"),
                             hazard_fcn,
                             "}",
                             paste0("typedef Rcpp::NumericVector(*hazard_ptr)(", hazard_args,");"),
                             "// [[Rcpp::export]]",
                             "Rcpp::XPtr<hazard_ptr> HAZARD_XPtr() {",
                             "return(Rcpp::XPtr<hazard_ptr>(new hazard_ptr(&HAZARD)));",
                             "}", sep = "\n")

        jacobian_code <- paste("// [[Rcpp::depends(Rcpp)]]",
                               "#include <Rcpp.h>",
                               "using namespace Rcpp;",
                               paste0("Rcpp::NumericMatrix JACOBIAN(",jacobian_args,") {"),
                               jacobian_fcn,
                               "}",
                               paste0("typedef Rcpp::NumericMatrix(*jacobian_ptr)(", jacobian_args,");"),
                               "// [[Rcpp::export]]",
        ...
        lna_pointers <- c(hazard_ptr = HAZARD_XPtr(), jacobian_prt = JACOBIAN_XPtr())
