...
#'  documentation for the \code{\link{forcing}} function).
...
#'  \item{timevarying}{logical indicating whether there are any smoothly
#'  time-varying covariates in the model (either time, or seasonality)}
#'  \item{timecode}{index of column in the time-varying covariate matrix for
#'  time (as a variable, not an index)} \item{tcovar_adjmat}{adjacency matrix
#'  indicating which rates need to be updated when a time-varying covariate
#'  value changes} \item{tcovar_changemat}{indicator matrix indicating which
#'  time-varying covariates change at which time in the tcovar matrix}
...
#'  #' @param step_size initial step size for ODE stepper. Adapted internally,
#'  but too large of an initial size can lead to failures in stiff systems.
#'@export
stem_dynamics <-
        function(rates,
                 parameters,
        ...
        if(!is.null(strata)) {
                ...
                for(k in seq_along(rates)) {
                        ...
                        if(!is.null(rates[[k]]$seasonality)) {
                                ...
                                # add the seasonality
                                rate_fcns[[k]]$unlumped <- paste(rate_fcns[[k]]$unlumped, rates[[k]]$seasonality$seasonality, sep = " + ")
                        }
        ...
        # compile LNA functions -- RESUME HERE
