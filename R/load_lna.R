...
#' @export
load_lna <- function(lna_rates, lna_scale, messages) {
        ...
        rate_terms      <- paste(paste("odeintr::rates[",0:(n_rates-1),"]", " = ", lna_rates$rates,";", sep = ""), collapse = "\n")
        ...
        if(lna_scale == "linear") {
                ...
                diffusion_ode   <- paste0("odeintr::diffusion_ode = arma::vectorise(odeintr::diffusion * odeintr::jacobian.t() + ",
                                          "arma::diagmat(odeintr::rates) + ",
                                          "odeintr::jacobian * odeintr::diffusion);")
        } else if(lna_scale == "log") {
                drift_ode    <- paste0("odeintr::drift_ode = arma::exp(-odeintr::drift) % odeintr::rates",
                                          " - 0.5 * arma::exp(-2*odeintr::drift) % odeintr::rates;")
                resid_ode    <- "odeintr::resid_ode = odeintr::jacobian * odeintr::resid;"
                diffusion_ode<- paste0("odeintr::diffusion_ode=arma::vectorise(odeintr::diffusion*odeintr::jacobian.t()+",
                                          "arma::diagmat(arma::exp(-2*odeintr::drift))* arma::diagmat(odeintr::rates) + ",
                                          "odeintr::jacobian * odeintr::diffusion);")
        }

        # dxdt strings
        dxdt_drift      <- paste("dxdt[", drift_inds, "] = odeintr::drift_ode[", seq_along(drift_inds) - 1, "];", collapse = "\n", sep = "")
        dxdt_resid      <- paste("dxdt[", resid_inds, "] = odeintr::resid_ode[", seq_along(resid_inds) - 1, "];", collapse = "\n", sep = "")
        dxdt_diffusion  <- paste("dxdt[", diffusion_inds, "] = odeintr::diffusion_ode[", seq_along(diffusion_inds)-1, "];", collapse = "\n", sep = "")
