...
#'@param joint_block_update should parameter blocks be updated jointly,
#'  defaults to TRUE.
#'
...
kernel <-
        function(method,
                 ...
                 mvnss_setting_list = NULL,
                 parameter_blocks = NULL,
                 joint_block_update = TRUE,
                 messages = TRUE) {

      if(!method %in% c( "mvn_rw", "mvn_g_adaptive", "afss", "harss", "mvnss")) {
            stop("The method for the MCMC kernel is not correctly specified.")
      }
              
      if(method != "mvnss" & !is.null(parameter_blocks)) {
            warning("Parameter blocking is only implemented for mvnss.")
      }
