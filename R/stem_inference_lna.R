...
stem_inference_lna <- function(stem_object,
        ...
        if(mcmc_kernel$method == "mvn_rw") {
            ...
            weight_update_interval <- mcmc_kernel$kernel_settings$weight_update_interval
            pcm_updates            <- mcmc_kernel$kernel_settings$pcm_updates
            if(mcmc_kernel$kernel_settings$pcm_adaptive_schedule == "sequential") d_schedule <- rev(seq_len(n_model_params))
            ...
            adaptation_shape_record[,,1] <- kernel_cov
              ...
              e                <- eigen(kernel_cov)
              interval_widths  <- e$eigenvalues * 7
              slice_directions <- e$eigenvectors
              ...
              slice_direction_record[,,1]  <- slice_directions
              adaptation_shape_record[,,1] <- kernel_cov
        }
        ...
        if(!is.null(stem_object$dynamics$tparam)) {
              ...
              if(tparam_update == "block") tparam_ess  <- 1
              ...
              if(!mcmc_restart) {
                    ...
                    for(s in seq_along(tparam)) {
                          ...
                          # indices
                          tparam[[s]]$col_ind   <- stem_object$dynamics$lna_rates$lna_param_codes[stem_object$dynamics$tparam[[s]]$tparam_name]
                          tparam[[s]]$tpar_inds <- findInterval(lna_times, tparam[[s]]$times, left.open = F) - 1
        ...
        if(!is.null(tparam)) {
              tparam_samples <- array(0.0, dim = c(n_times, length(tparam), 1 + floor(iterations / thin_params)))
        } else {
              tparam_samples <- NULL
        }
        ...
        for(iter in (seq_len(iterations) + 1)) {
                ...
                if(mcmc_kernel$method == "c_rw") {
                        ...
                        if(iter == stop_adaptation) {
                                kernel_cov <- diag(sqrt(proposal_scaling)) %*% kernel_cov %*% diag(sqrt(proposal_scaling))
                        ...
                        for(s in component_order) {
                                ...
                                if(iter < stop_adaptation) {
                                        proposal_scaling[s] <- min(exp(log(proposal_scaling[s]) +
                                                                           adaptations[iter] * (min(exp(acceptance_prob),1) - target_c)),
                                                                   max_scaling)
                                }
                        }
                      ...
                      if(mcmc_kernel$kernel_settings$pcm_adaptive_schedule == "random") {
                            d_schedule <- sample.int(n_model_params, pcm_updates, replace = FALSE, prob = comp_weights, useHash = FALSE)
                      } 
                      ...
                      for(direction in d_schedule) {
                            ...
                            # sample a direction
                            proposals_c[direction] <- proposals_c[direction] + 1
                            
                            # propose new parameters
                            pcm_adaptive(params_prop      = params_prop_est,
                                         params_cur       = model_params_est,
                            ...
                            if(acceptance_prob >= 0 || acceptance_prob >= log(runif(1))) {
                                  ...
                                  ### ACCEPTANCE
                                  acceptances_c[direction] <- acceptances_c[direction] + 1    # increment acceptances
                            ...
                            if(iter < stop_adaptation) {
                                  ...
                                  proposal_scaling[direction] <- min(
                                        exp(log(proposal_scaling[direction]) + 
                                                  adaptations[iter] * (min(exp(acceptance_prob), 1) - acceptances_c[direction] / proposals_c[direction]) +
                                                  adaptations[iter] * (acceptances_c[direction] / proposals_c[direction] - target_c)),
                                        max_scaling)
                            }
                      }
                      ...
                      if(iter < stop_adaptation) { 
                            ...
                            update_princomps(eigenvalues    = eigenvalues,
                                             eigenvectors   = eigenvectors,
                ...
                if(iter %% thin_params == 0) {
                        ...
                        if(mcmc_kernel$method == "c_rw_adaptive") {
                              ...
                              adaptation_scale_record[, param_rec_ind] <- proposal_scaling
                              adaptation_shape_record[,,param_rec_ind] <- kernel_cov
                              eigenvalue_record[,param_rec_ind]        <- eigenvalues
                              eigenvector_record[,,param_rec_ind]      <- eigenvectors
                              
                              if(messages && iter<stop_adaptation) cat(
                                    paste0("Iteration: ", iter),
                                    paste0("Directional acceptances: ", acceptances_c),
                                    paste0("Directional acceptance rates: ", acceptances_c/(iter-1)),
                                    file = status_file, sep = "\n", append = TRUE)
                        }
        ...
        if(!is.null(tparam)) {
              ...
              if(tparam_update == "block") stem_object$results$tparam_ess_record <- tparam_ess_record
        }
